...
variable "metrics_server_version" {
  type        = string
  description = "The version of the Metrics Server Addon. Specify a version to enable the addon, or leave null to disable the addon."
...
variable "eks_node_groups" {
  description = <<-EOT
    EKS managed node groups. Optional `eks_ami_id`, `eks_ami_type`, and `eks_ami_release_version` override the cluster-level `eks_ami_*` variables for that node group only. For Graviton, set e.g. `eks_ami_type = "AL2023_ARM_64_STANDARD"` and ARM instance types on the groups that should use it.
  EOT
  type = list(object({
    name                    = string
    min_size                = string
